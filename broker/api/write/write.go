...
func (m *MetricWriteAPI) Sum(c *gin.Context) {
	...
	for i := 0; i < param.Count; i++ {
		var metrics []*protoMetricsV1.Metric
		for j := 0; j < 4; j++ {
			for k := 0; k < 20; k++ {
				metric := &protoMetricsV1.Metric{
					...
					SimpleFields: []*protoMetricsV1.SimpleField{
						{Name: "f2", Type: protoMetricsV1.SimpleFieldType_DELTA_SUM, Value: 1.0},
					},
					Tags: tag.KeyValuesFromMap(map[string]string{
						"host":      "1.1.1." + strconv.Itoa(i),
