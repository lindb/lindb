...
func TestLeafTask_Process_Fail(t *testing.T) {
	...
	err = processor.Process(context.TODO(), &protoCommonV1.TaskRequest{PhysicalPlan: plan, Payload: []byte{1, 2, 3}})
...
func TestLeafTask_Suggest_Process(t *testing.T) {
	...
	// test unmarshal err
	err := processor.Process(context.TODO(), &protoCommonV1.TaskRequest{
		PhysicalPlan: plan,
