...
func TestLeafTaskDispatcher_Dispatch(t *testing.T) {
	...
	leafTaskDispatcher.Dispatch(context.TODO(), server, &protoCommonV1.TaskRequest{PhysicalPlan: []byte{1, 1, 1}})
}

func TestIntermediateTaskDispatcher_Dispatch(t *testing.T) {
	dispatcher := NewIntermediateTaskDispatcher()
	dispatcher.Dispatch(context.TODO(), nil, &protoCommonV1.TaskRequest{PhysicalPlan: []byte{1, 1, 1}})
}
