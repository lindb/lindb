...
//go:generate mockgen -source=./partition.go -destination=./partition_mock.go -package=storage

// Partition represents a time based partition, there are some segments in a interval partition.
...
func newSegment(shard Shard, segmentName string, interval timeutil.Interval) (Partition, error) {
...
// BaseTime returns segment base time
func (s *partition) BaseTime() int64 {
	return s.baseTime
}

// GetDataFamilies returns data family list by time range, return nil if not match
func (s *partition) GetDataFamilies(timeRange timeutil.TimeRange) []DataFamily {
...
// NeedEvict checks segment if it can evict, long term no read operation.
func (s *partition) NeedEvict() bool {
	s.mutex.Lock()
...
// EvictFamily evicts data family.
func (s *partition) EvictFamily(familyTime int64) {
	calc := s.interval.Calculator()
...
// GetOrCreateDataFamily returns the data family based on timestamp.
func (s *partition) GetOrCreateDataFamily(timestamp int64) (DataFamily, error) {
	calc := s.interval.Calculator()
...
func (s *partition) getOrLoadFamily(familyName string, familyTime int) DataFamily {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *partition) initDataFamily(familyTime int, family kv.Family) DataFamily {
	calc := s.interval.Calculator()
	// create data family
