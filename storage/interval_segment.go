...
//go:generate mockgen -source=./interval_segment.go -destination=./interval_segment_mock.go -package=storage
...
type IntervalSegment interface {
	...
	GetOrCreateSegment(segmentName string) (Partition, error)
...
func newIntervalSegment(shard Shard, interval option.Interval) (segment IntervalSegment, err error) {
	...
	return &intervalSegment{
		...
		shard:    shard,
		interval: interval,
		segments: make(map[string]Partition),
		logger:   logger.GetLogger("TSDB", "IntervalSegment"),
	}, nil
}

// GetOrCreateSegment creates new segment if not exist, if exist return it
func (s *intervalSegment) GetOrCreateSegment(segmentName string) (Partition, error) {
	s.mutex.Lock()
...
func (s *intervalSegment) getOrLoadSegment(segmentName string) (Partition, error) {
	s.mutex.Lock()
	defer s.mutex.Unlock()
