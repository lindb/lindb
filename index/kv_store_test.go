...
func TestIndexKVStore(t *testing.T) {
	...
	for bucketID := uint32(0); bucketID < 100; bucketID++ {
		...
		for i := 0; i < 1000; i++ {
			...
			id, err0 := indexStore.GetOrCreateValue(bucketID, scratch[:], func() uint32 {
				seq++
				return seq
			})
	...
	test := func() {
		_ = buckets.WalkEntry(func(key uint32, value map[string]uint32) error {
			for k, v := range value {
				id, err0 := indexStore.GetOrCreateValue(key, strutil.String2ByteSlice(k), func() uint32 {
					panic("err")
				})
				assert.NoError(t, err0)
				assert.Equal(t, id, v)
...
func TestIndexKVStore_Find(t *testing.T) {
	...
	for idx, key := range keysString {
		v, err := indexStore.GetOrCreateValue(100, []byte(key), func() uint32 { return uint32(idx) })
		assert.NoError(t, err)
...
func TestIndexKVStore_Flush_Error(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			prepare: func() {
				store1.immutable = imap.NewIntMap[map[string]uint32]()
				store1.immutable.Put(10, map[string]uint32{"test": 1000})
