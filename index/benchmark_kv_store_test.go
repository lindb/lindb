...
func BenchmarkIndexKVStore(b *testing.B) {
	...
	for bucketID := uint32(0); bucketID < 100; bucketID++ {
		...
		for i := 0; i < 1000; i++ {
			...
			id, _, _ := indexStore.GetOrCreateValue(bucketID, scratch[:], func() (uint32, error) {
				seq++
				return seq, nil
			})
	...
	for i := 0; i < b.N; i++ {
		_ = buckets.WalkEntry(func(key uint32, value map[string]uint32) error {
			for k := range value {
				_, _, _ = indexStore.GetOrCreateValue(key, strutil.String2ByteSlice(k), func() (uint32, error) {
					panic("err")
				})
			}
			return nil
