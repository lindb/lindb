...
func TestShard_New(t *testing.T) {
	...
	db.EXPECT().MemMetaDB().Return(nil).AnyTimes()
...
func TestShard_Close(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			prepare: func() {
				memIndexDB.EXPECT().Notify(gomock.Any()).DoAndReturn(func(event any) {
