...
func TestDataSegment_New(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:        "create dataSegment successfully and set rollup",
			segmentName: segmentName,
			prepare: func() {
		...
		{
			name:        "parse dataSegment name err",
			segmentName: "xx",
			wantErr:     true,
		},
		{
	...
	for _, tt := range cases {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			s, err := newDataSegment(shard, tt.segmentName, interval)
			if ((err != nil) != tt.wantErr && s == nil) || (!tt.wantErr && s == nil) {
				t.Errorf("newDataSegment() error = %v, wantErr %v", err, tt.wantErr)
			}
...
func TestDataSegment_GetOrCreateDataFamily(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	cases := []struct {
		name      string
		timestamp string
		prepare   func(seg *dataSegment)
		wantErr   bool
	}{
		{
			name:      "dataSegment time != base time",
		...
		},
		{
			...
			prepare: func(_ *dataSegment) {
				newDataFamilyFunc = func(shard Shard, _ DataSegment,
...
func TestDataSegment_GetDataFamilies(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	cases := []struct {
		...
		},
		{
			...
			prepare: func(_ *dataSegment) {
				...
				newDataFamilyFunc = func(shard Shard, _ DataSegment, interval timeutil.Interval,
...
func TestDataSegment_Close(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer func() {
		kv.InitStoreManager(nil)
		ctrl.Finish()
...
func TestDataSegment_NeedEvict(t *testing.T) {
	interval := timeutil.Interval(10 * 1000)
	s := &dataSegment{interval: interval}
